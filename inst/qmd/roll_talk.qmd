...
$$
\begin{aligned}
\text{mean}&=\frac{1}{n}\sum_{i=1}^{n}{x_{i}}
\end{aligned}
$$
...
mean_mat <- na.omit(roll::roll_mean(x, width = 15))
mean_mlt <- data.table::melt(data.table::as.data.table(mean_mat, keep.rownames = TRUE), id.vars = "rn")
mean_mlt[ , rn := as.Date(rn)]

mean_plt <- plot_ts(mean_mlt, x = "rn", title = "Rolling means")
